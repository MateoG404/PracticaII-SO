...
{
    // calcular el índice en donde se va escribir el registro no mayor a 1160
    int key = data->sourceid;
    unsigned int index = hash(key);
    travel_t record;//almacenar en memoria el registro que se va a leer en el archivo binario
    ...
    {   
        ...
        while(record.next_offset!=-1){//,mientras la siguiente posicion en el archivo no sea -1
            fseek(file, record.next_offset,SEEK_SET);//continuamos moviendo el puntero en el archivo
            fread(&record,sizeof(travel_t),1,file);//leemso en record lo que vamos encontrando
        }
...
float search(int data[3])
{
    ...
    if (file == NULL)//Si el archivo no existe es necesario crear e insertar la estructura hash
    {
        ...
        result = find_record(file, data[0], data[1], data[2]);//se busca el registro en la tabla hash
        return result;
